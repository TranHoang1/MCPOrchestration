...
class KbEntryRepositoryImpl(
    ...
    private fun setEntryParams(stmt: PreparedStatement, entry: KbEntry) {
        ...
        setEncryptedBusinessRules(stmt, 6, entry.businessRules)
        stmt.setString(7, entry.maskedFull)
        stmt.setInt(8, entry.brSensitivityLevel.level)
        stmt.setString(9, entry.contentHash)
    }

    private fun setEncryptedBusinessRules(
        stmt: PreparedStatement,
        index: Int,
        businessRules: String?
    ) {
        if (businessRules != null) {
            stmt.setBytes(index, encryptionService.encrypt(businessRules))
        } else {
            stmt.setNull(index, Types.BINARY)
        }
    }

    private fun mapResults(rs: ResultSet): List<KbEntry> {
        val results = mutableListOf<KbEntry>()
        while (rs.next()) results.add(mapRow(rs))
        return results
    }

    private fun mapRow(rs: ResultSet): KbEntry {
        ...
        return KbEntry(
            ...
            brSensitivityLevel = BrSensitivityLevel.fromLevel(
                rs.getInt("br_sensitivity_level")
            ),
            contentHash = rs.getString("content_hash"),
            createdAt = rs.getTimestamp("created_at").toKotlinInstant(),
            updatedAt = rs.getTimestamp("updated_at").toKotlinInstant(),
            lastSyncedAt = rs.getTimestamp("last_synced_at")?.toKotlinInstant()
        )
    }
    ...
    private inline fun <T> executeWithErrorHandling(
        ...
        block()
    } catch (e: KbStoreException) {
        throw e
    } catch (e: Exception) {
        throw KbStoreException.RepositoryException(
            "KbEntryRepository.$operation failed: ${e.message}", e
        )
    }
    ...
    companion object {
        ...
        private const val FIND_BY_ISSUE_KEY =
            "SELECT * FROM kb_entries WHERE issue_key = ?"

        private const val FIND_BY_PROJECT_KEY =
            "SELECT * FROM kb_entries WHERE project_key = ?"

        private const val FIND_BY_HASH =
            "SELECT * FROM kb_entries WHERE project_key = ? AND content_hash = ?"

        private const val UPDATE_SYNCED_AT =
            "UPDATE kb_entries SET last_synced_at = ? WHERE issue_key = ?"

        private const val DELETE_SQL =
            "DELETE FROM kb_entries WHERE issue_key = ?"
    }
}
