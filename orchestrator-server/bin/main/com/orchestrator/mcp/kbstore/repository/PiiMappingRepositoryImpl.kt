...
class PiiMappingRepositoryImpl(
    ...
    private inline fun <T> executeWithErrorHandling(
        ...
        throw KbStoreException.RepositoryException(
            "PiiMappingRepository.$operation failed: ${e.message}", e
        )
    }
    ...
    companion object {
        ...
        private const val FIND_BY_ISSUE_KEY =
            "SELECT * FROM pii_mapping WHERE issue_key = ?"

        private const val DELETE_BY_ISSUE_KEY =
            "DELETE FROM pii_mapping WHERE issue_key = ?"
    }
}
