...
/**
 * SQL constants for RLS migration.
 * Separated from RlsDatabaseInitializer to respect 200-line file limit.
 */

internal val CREATE_ROLES = """
    DO ${'$'}${'$'}
    BEGIN
...
internal val ENABLE_RLS_KB_ENTRIES = """
    ALTER TABLE kb_entries ENABLE ROW LEVEL SECURITY;
    ALTER TABLE kb_entries FORCE ROW LEVEL SECURITY;
""".trimIndent()

internal val CREATE_KB_POLICIES = """
    DO ${'$'}${'$'}
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies WHERE policyname = 'policy_kb_admin_all'
        ) THEN
            CREATE POLICY policy_kb_admin_all ON kb_entries
                FOR ALL TO kb_admin USING (true) WITH CHECK (true);
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies WHERE policyname = 'policy_kb_developer_select'
        ) THEN
            CREATE POLICY policy_kb_developer_select ON kb_entries
                FOR SELECT TO kb_developer USING (true);
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies WHERE policyname = 'policy_kb_viewer_select'
        ) THEN
            CREATE POLICY policy_kb_viewer_select ON kb_entries
                FOR SELECT TO kb_viewer USING (true);
        END IF;
    END
    ${'$'}${'$'};
""".trimIndent()

internal val CREATE_SECURITY_VIEWS = """
    CREATE OR REPLACE VIEW kb_entries_developer_view
        WITH (security_barrier = true) AS
    SELECT id, issue_key, public_content, technical_content,
...
internal val ENABLE_RLS_PII_MAPPING = """
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies WHERE policyname = 'policy_pii_admin_only'
        ) THEN
